            ...
            out.append(
                _event(
                    ...
                    extra={
                        ...
                        "storm_name": (props.get("eventname") or "") if code == "TC" else None,
...
def merge(events: list[dict]) -> list[dict]:
    ...
    for event in events:
        ...
        for key in ("latitude", "longitude", "location", "url", "magnitude", "updated"):
...
def _refresh_source(name: str, client: httpx.Client) -> None:
    ...
    try:
        ...
        if previous and previous.get("events") is not None and previous["status"] != "unavailable":
            previous.update(status="stale", error=type(exc).__name__)
        else:
            _cache[name] = {
...
def _due(force: bool) -> list[str]:
    ...
    def needs_refresh(name: str) -> bool:
        ...
        age = now - entry["at"]
        if entry["status"] != "ok":
            return age > 60  # retry a failed feed after a minute, not on every request
        return age > ttl
...
def _refresh(due: list[str]) -> None:
    ...
    try:
        ...
        for future in late:
            ...
            previous = _cache.get(name)
            if previous and previous.get("events") is not None and previous["status"] != "unavailable":
                previous.update(status="stale", error="Timeout")
            else:
                _cache[name] = {"at": time.time(), "events": [], "meta": {},
...
def get_live(force: bool = False) -> dict[str, Any]:
    ...
    return {"events": events, "sources": sources, "layers": layers, "fetched_at": _iso(_now())}


def clear_cache() -> None:
    """Drop every cached source (used by tests)."""
    _cache.clear()
