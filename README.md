...
| `/map` | World Map | **Live now**: current events from USGS, GDACS, NASA EONET on a full-width satellite map (plate boundaries, cyclone tracks where GDACS or EONET publish one) with a Live Events list of what is in view. Severity: GDACS alert Green/Orange/Red = Moderate/High/Very High; USGS quakes without a GDACS alert by magnitude (M4.5–5.9 / 6.0–6.9 / 7.0+); anything else is "Not rated". Each feed's last good response is kept on disk (`LIVE_CACHE_DIR`) and shown as cached when the feed is down. **Historical**: decade slider; USGS earthquakes and IBTrACS cyclones at exact positions; EM-DAT floods at country centres |
